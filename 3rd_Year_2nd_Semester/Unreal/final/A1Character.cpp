...
void AA1Character::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindTouch(IE_Pressed, this, &AA1Character::TouchStarted);
	PlayerInputComponent->BindTouch(IE_Released, this, &AA1Character::TouchStopped);
...
{
	// If A1 is added to a project via 'Add Feature' in the Unreal Editor the dependency on HeadMountedDisplay in A1.Build.cs is not automatically propagated
...
void AA1Character::TouchStarted(ETouchIndex::Type FingerIndex, FVector Location)
{
		Jump();
}

void AA1Character::TouchStopped(ETouchIndex::Type FingerIndex, FVector Location)
{
		StopJumping();
}
